...
import sys
sys.path.append("/home/tfguan/work/chip_rel/code")
sys.path.append("/home/tfguan/work/chip_rel")
print(sys.path)
import numpy as np
import torch
...
def get_args():
    ...
    parser.add_argument("--test_src", default=None, type=str, required=False, help="test file path")
...
def bulid_test_dataset(args, spo_config, reader, tokenizer, debug=False):
    test_src = args.test_src  # 或"raw_data/test1.json"
...
def main():
    ...
    tokenizer = MBartTokenizer.from_pretrained(args.bert_model, do_lower_case=True)
    ...
    if args.train_mode != "test":
        ...
        eval_examples, data_loaders, tokenizer = bulid_train_dataset(args, spo_conf, reader, tokenizer,
                                                                     debug=args.debug)
        for step, batch in tqdm(enumerate(data_loaders[-1]), mininterval=5, leave=False, file=sys.stdout):
            print(step, batch)
            for i in batch:
                print(i)
            break
