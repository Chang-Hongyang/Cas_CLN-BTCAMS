...
class ConditionalLayerNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-12):
        ...
        self.beta_dense_ = Linear(hidden_size, hidden_size, bias=False)
        self.gamma_dense_ = Linear(hidden_size, hidden_size, bias=False)

    def forward(self, x, cond, is_sub=True):  #[bsize,seqlen, hsize], [bsize, hsize*2]
        cond = cond.unsqueeze(1)
        if is_sub:
            beta = self.beta_dense(cond)
            gamma = self.gamma_dense(cond)
        else:
            beta = self.beta_dense_(cond)
            gamma = self.gamma_dense_(cond)
