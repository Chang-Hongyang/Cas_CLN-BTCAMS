...
if __name__ == '__main__':
    # tokenizer = BertTokenizer.from_pretrained('../../pretrained_model/chinese_roberta_wwm_large_ext_pytorch', do_lower_case=True)
    tokenizer = MBartTokenizer.from_pretrained('../../pretrained_model/mbart-large-cc25')
    ...
    # text_raw = "1%～2%ALL 患者 有Burkitt淋巴瘤 的形态学及 免疫学特征（即FAB形态学分类中的L3型），" \
    #            "常伴有腹部或其余部位的包块性疾病，可以将之视为极晚期Burkitt淋巴瘤，" \
    #            "对这类患儿应采用晚期Burkitt淋巴瘤的治疗方案。"
    text_raw = 'It envisioned a sprawling cultural center and Fordham campus on the squalid West Side , a civic plaza in Downtown Brooklyn that Robert Moses promised would rival the Piazza San Marco in Venice , a grand development over the Sunnyside railroad yards in Queens , and a Palace of Progress devoted to world trade atop a reconstructed Pennsylvania Station .'
    sub_text = []
    buff = ""
    flag_en = False
    flag_digit = False
    ...
    print(tok_to_orig_start_index)
    print(tok_to_orig_end_index)
