...
from code.utils.data_utils import covert_to_tokens, search_spo_index
from code.utils.data_utils import sequence_padding
    ...
    def _read(self, filename, data_type):
        ...
        with open(filename, 'r') as fr:
            ...
            for line in tqdm(fr.readlines()):
                ...
                spo_list = data_line['relationMentions']
                ...
                for spo in spo_list:
                    ...
                    # _subject_sub_tokens = covert_to_tokens(subject, tokenizer=self.tokenizer)
                    # _object_sub_tokens = covert_to_tokens(object, tokenizer=self.tokenizer)
                    #
                    # subject_sub_tokens, object_sub_tokens  = [], []
                    # for s, o in zip(_subject_sub_tokens, _object_sub_tokens):
                    #     if '▁' in s:
                    #         if s.replace('▁', ''): subject_sub_tokens.append(s)
                    #     else: subject_sub_tokens.append(s)
                    #     if  '▁' in o:
                    #         if o.replace('▁', ''): object_sub_tokens.append(o)
                    #     else: object_sub_tokens.append(o)

                    # print('*' * 10)
                    # print(subject_sub_tokens, object_sub_tokens, tokens)
                    subject_start, object_start = search_spo_index(tokens, subject_sub_tokens, object_sub_tokens)
                    # print(subject_start, object_start)
                    # print('*' * 10)

                    predicate_label = self.spo_conf[predicate]

                    if subject_start != -1 and object_start != -1:
                        s = (subject_start, subject_start + len(subject_sub_tokens) - 1)  #[sub_start, sub_end]
                    ...
                    for s in spoes.keys():
                        examples.append(
                            Example(
                                p_id=p_id,
                                context=text_raw,
                                choice_sub=s,  #[sub_start, ... , sub_end]
                                tok_to_orig_start_index=tok_to_orig_start_index,
                                tok_to_orig_end_index=tok_to_orig_end_index,
                                bert_tokens=tokens,
            ...
            for example in examples:
                ...
                if self.is_train:
                    if spoes:
                        ...
                        # 非随机选一个subject
                        subject_ids = example.choice_sub  #[sub_start, ... , sub_end]
