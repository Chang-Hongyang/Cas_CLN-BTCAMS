...
from code.utils.data_utils import batch_gather, batch_gather_sub
from code.utils.layernorm import ConditionalLayerNorm
# from code.utils.modeling_nezha import BertModel, BertPreTrainedModel
from transformers import MBartModel, MBartConfig, PreTrainedModel
...
class ERENet(nn.Module):
    ...
    def __init__(self, config, classes_num):
        ...
        self.bert = MBartModel.from_pretrained(config)
        config = MBartConfig.from_pretrained(config)
        self.token_entity_emb = nn.Embedding(num_embeddings=2, embedding_dim=config.hidden_size,
                                             padding_idx=0)
        config.layer_norm_eps = 1e-12
        # 修改成原论文主语与encoder输出相加
    ...
    def forward(self, q_ids=None, passage_ids=None, segment_ids=None, token_type_ids=None,
                subject_ids=None,subject_labels=None,object_labels=None, eval_file=None,is_eval=False):
        ...
        if not is_eval:
            ...
            # sub_start_encoder = batch_gather(bert_encoder, subject_ids[:, 0])  #bert_en[bsize, seqlen, hsize]; sub_ids[bsize, 1]
            # sub_end_encoder = batch_gather(bert_encoder, subject_ids[:, 1])  #bert_en[bsize, seqlen, hsize]; sub_ids[bsize, 1]
            # subject = torch.cat([sub_start_encoder, sub_end_encoder], 1)
            # context_encoder = self.LayerNorm(bert_encoder, subject)
            sub_len = subject_ids[:, 1] - subject_ids[:, 0] + 1
            sub_average = batch_gather_sub(bert_encoder, subject_ids[:, 0], sub_len)
            context_encoder = bert_encoder + sub_average
            ...
            for i in range(len(bert_encoders_)):
                ...
                context_encoder = bert_encoders + (sub_start_encoder+sub_end_encoder)/2
                po_pred = self.po_dense(context_encoder).reshape(context_encoder.size(0), -1, self.classes_num, 2)
                if flag:
